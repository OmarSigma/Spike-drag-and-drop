...
window.PdfViewerSignature = {
	...
	setReadOnlyThemeVisual: function (divId) {
		this.viewer.toolbar.showToolbarItem(["OpenOption"
			, "AnnotationEditTool"
		...
		var toolbar = document.getElementById(divId).getElementsByClassName("e-toolbar-left")[0];
	...
	loadPdfViewer: function (divId, urlApi) {
		...
		if (!this.isPdfViewerInyect)
			this.injectPdfViewer();
	...
	addSignatureFunction: function (page,x,y, signer) {
		...
		window.PdfViewerSignature.viewer.formDesignerModule.addFormField('SignatureField', signatureField01);
...
window.PdfViewerSignature.loadPdfViewer("PdfViewer","https://ej2services.syncfusion.com/production/web-services/api/pdfviewer");
